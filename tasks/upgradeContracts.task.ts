...
import { ADMIN_ADDRESS, AvatarName, AvatarSymbol, DEPLOYER_IS_ADMIN, KeyCardName, KeyCardSymbol, MAXIMUM_AVATAR_TOKENS } from "../Constants";
...
const upgradeFunction = async (
    ...
    const controller = (await upgrades.upgradeProxy(oldControllerProxy,controllerFactory, {
    ...
    const avatarInstance = (await upgrades.upgradeProxy(oldAvatarProxy, avatarFactory)) as Avatar;
    ...
    const keyCardInstance = (await upgrades.upgradeProxy(oldKeyCardProxy, keyCardFactory)) as KeyCard;
    console.log(
      `Waiting for ${contractsName.KEY_CARD} upgrade confirmation...`
    );
